...
#Delete SRA files
if DPP_configuration.lowmemory == True:
...
base_name = DPP_configuration.refGenomePath.replace('temporary/bowtie2/refGenomes/','').replace('.fasta','')
...
for file in featurecounts_input:
	if '.bam' in file and '.bam.' not in file:
		...
		os.system('featureCounts -a ' + DPP_configuration.annotationPath + ' -F "SAF" -o temporary/feature_counts/output/' + file[:-4] + '.txt temporary/samtools/bam_files_sorted/' + file)
